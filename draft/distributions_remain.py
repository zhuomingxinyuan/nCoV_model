...
def plot_curves(days, batch_list, take_list, better_prob, worse_prob, better_mean, better_std, worse_mean, worse_std):
...
def intervention_take_out_patients(date, take_list, batch_list):
...
if __name__ == '__main__':
	...
	#### 医院抽走人数 ##########
	take_list=np.zeros(days)*batch_population #list(range(days))
	take_tuple_list=[(2,10),(5,300),(6,50),(10,20),(15,50)] #第5天抽走300人，第10天抽走20人。
	for take_date, take_number in take_tuple_list:
		for i in range(take_date, days):
			if batch_list[i]-take_number > 0: ### batch list 算蓝线
				batch_list[i]-=take_number
			else:
				batch_list[i]=0
		take_list[take_date]=take_number   ### take list 在plot_curve里用来从remain里减去黄线。

	better_list, worse_list, better_cumu_list, worse_cumu_list, remain_list=plot_curves(days, batch_list, take_list, better_prob, worse_prob, better_mean, better_std, worse_mean, worse_std)
