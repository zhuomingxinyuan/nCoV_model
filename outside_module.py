...
def intervention_take_out_patients(take_out_tuple_list, batch_list):
	for date, take_number in take_out_tuple_list:
		for i in range(date, len(batch_list)):
			batch_list[i]-=take_number
	return batch_list 

def intervention_take_out_patients_change_prob_lists(date, remain_before_date_prob, better_prob_list, worse_prob_list, better_prob_list_original, worse_prob_list_original):
	for i in range(date, len(better_prob_list)):
		better_prob_list[i]=better_prob_list_original[i]*remain_before_date_prob
		# better_prob_list[i]*=remain_before_date_prob
	for i in range(date, len(worse_prob_list)):
		worse_prob_list[i]=worse_prob_list_original[i]*remain_before_date_prob
...
if __name__ == '__main__':
	...
	take_out_tuple_list=[(10, 2), (15,10), (17, 5)]
	take_dates=[x[0] for x in take_out_tuple_list]
	take_dates.sort()
	take_dates=np.unique(take_dates)
	batch_list=intervention_take_out_patients(take_out_tuple_list, batch_list) # how intervention happens once. Need to put in a loop to intervene everyday.
	...
	for take_out_date in take_dates:
		...
		better_prob_list, worse_prob_list=intervention_take_out_patients_change_prob_lists(take_out_date, remain_before_date_prob, better_prob_list, worse_prob_list, better_prob_list_original, worse_prob_list_original)
